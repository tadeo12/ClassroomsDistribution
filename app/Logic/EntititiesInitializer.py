...
def createEntitiesFromJson(json_data):
    ...
    for classroom_data in data['classrooms']:
        classroom = Classroom(
        classroom_data['name'], 
        classroom_data['capacity'], 
        places_dictionary[classroom_data['place']]
        )
        classrooms.append(classroom)
        availableSlots = set(tuple(slot) for slot in classroom_data.get("availableTimeSlots", []))
        classroomAvailability[classroom] = availableSlots
    ...
    Resource._counter = 0     
    resources = generateResources(classrooms,classroomAvailability)
